...
func (s *PersistentShell) execCommand(command string, timeout time.Duration, ctx context.Context) commandResult {
	...
	tempDir := os.TempDir()
	stdoutFile := filepath.Join(tempDir, fmt.Sprintf("localsandbox-stdout-%d", time.Now().UnixNano()))
	stderrFile := filepath.Join(tempDir, fmt.Sprintf("localsandbox-stderr-%d", time.Now().UnixNano()))
	statusFile := filepath.Join(tempDir, fmt.Sprintf("localsandbox-status-%d", time.Now().UnixNano()))
	cwdFile := filepath.Join(tempDir, fmt.Sprintf("localsandbox-cwd-%d", time.Now().UnixNano()))
